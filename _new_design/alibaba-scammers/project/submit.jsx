...
function Submit({ navigate }) {
  ...
  const [data, setData] = React.useState({
    ...
    quantity: '', totalPaid: '', currency: 'USD',
    industry: '',
    story: '',
  });
      ...
      <div className="grid-2">
        ...
        <div className="field">
          ...
          <select className="select" value={data.industry} onChange={set('industry')}>
            ...
            {window.INDUSTRIES.map((i) => <option key={i}>{i}</option>)}
          </select>
        </div>
      </div>
      <div className="grid-2">
        ...
        <div className="field">
          ...
          <select className="select" value={data.currency} onChange={set('currency')}>
            {window.CURRENCIES.map((c) => <option key={c}>{c}</option>)}
          </select>
        </div>
      </div>
    </div>
...
function StepStory({ data, set }) {
  ...
  return (
    <div>
      ...
      <div className="field">
        <textarea className="textarea" value={data.story} onChange={set('story')}
  ...
  return (
    <div>
      ...
      <div>
        ...
        <Field label="Total paid" value={data.totalPaid ? `${window.formatMoney(Number(data.totalPaid), data.currency)} ${data.currency}` : ''} onEdit={() => jump(1)} />
      ...
      <section style={{ paddingTop: 80, paddingBottom: 100, textAlign: 'center' }}>
        <div className="container-narrow">
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 32, flexWrap: 'wrap' }}>
            <button className="btn btn-primary" onClick={() => navigate('browse')}>Browse other reports</button>
