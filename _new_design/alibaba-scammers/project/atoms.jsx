...
function Brand({onClick}) {
  return (
    <button onClick={onClick} className="brand" style={{background: "none", border: "none", padding: 0}}>
      <BrandMark />
      <span>
        Alibaba scammerz<span style={{color: "var(--accent-ink)"}}>.wall</span>
      </span>
    </button>
  );
}
...
function Nav({route, navigate}) {
  const link = (label, target) => (
    <button onClick={() => navigate(target)} className={`nav-link ${route.name === target ? "active" : ""}`}>
      {label}
    </button>
  );
  return (
    <header className="nav">
      <div className="container nav-inner">
        ...
        <nav className="nav-links">
          ...
          <button className="btn btn-accent" style={{padding: "10px 16px", fontSize: 14, marginLeft: 6}} onClick={() => navigate("submit")}>
            Share Your Story
          </button>
        </nav>
      </div>
...
function Footer({navigate}) {
  return (
    ...
    >
      <div className="container" style={{display: "flex", flexWrap: "wrap", gap: 24, justifyContent: "space-between", alignItems: "flex-start"}}>
        <div style={{maxWidth: 360}}>
          ...
          <p className="muted small" style={{marginTop: 12, lineHeight: 1.55}}>
            A community-run wall of fraud reports about bad sellers on overseas wholesale marketplaces. We are not affiliated with Alibaba Group or any other
            platform. Reports reflect the experiences of the buyers who submitted them.
          </p>
        </div>
      ...
      >
        <span className="muted small">© 2026 alibaba scammer report — community led project.</span>
        <span className="muted small" style={{fontFamily: "var(--mono)"}}>
          Anonymous · Free · Moderated
        </span>
      </div>
    </footer>
  );
    ...
    <div>
      ...
      <ul style={{listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 8}}>
        {links.map((l, i) => (
          <li key={i}>
            {l.href ? (
              <a className="btn-link" href={l.href}>
                {l.label}
              </a>
            ) : (
              <button className="btn-link" onClick={() => navigate(l.go)}>
                {l.label}
              </button>
            )}
          </li>
  ...
  switch (name) {
    ...
    case "eye-off":
      return (
        <svg {...props}>
          <path d="M3 3l18 18" />
          <path d="M10.6 6.1A10.5 10.5 0 0 1 12 6c5 0 9 4 10 6-.4.8-1.2 2-2.4 3.2" />
          <path d="M6.6 6.6C4 8.2 2.5 10.6 2 12c1 2 5 6 10 6 1.4 0 2.7-.3 3.8-.8" />
    ...
    case "megaphone":
      return (
        <svg {...props}>
          <path d="M3 11v2a3 3 0 0 0 3 3h1l3 5 4-1-1-4 8-3V8l-8-3 1-4-4-1-3 5H6a3 3 0 0 0-3 3z" />
        </svg>
      );
    case "check":
      return (
