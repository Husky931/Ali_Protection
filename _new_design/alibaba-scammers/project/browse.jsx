      ...
      <section style={{ paddingTop: 56, paddingBottom: 30, borderBottom: '1px solid var(--line)' }}>
        <div className="container">
          ...
          <h1 style={{ fontSize: 'clamp(32px, 4.5vw, 48px)', letterSpacing: '-.03em', marginTop: 10, lineHeight: 1.05 }}>
            {window.REPORTS.length} reports of bad <span className="marker" style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontWeight: 600 }}>Alibaba</span> deals.
          </h1>
      ...
      <section style={{ paddingTop: 30, paddingBottom: 60 }}>
        <div className="container">
          <div className="muted small" style={{ marginBottom: 16 }}>
            ...
            {industry !== 'All' ? <> in <strong style={{ color: 'var(--ink)' }}>{industry}</strong></> : null}
          </div>
          {filtered.length === 0 ? (
            <div style={{ padding: '60px 20px', textAlign: 'center', background: 'var(--card)', border: '1px dashed var(--line-2)', borderRadius: 16 }}>
              ...
              <button className="btn btn-accent" style={{ marginTop: 16 }} onClick={() => navigate('submit')}>Share Your Story</button>
            </div>
          ) : (
